...
const Dashboard1 = () => {
    ...
    const fetchData = async () => {    
      try {
        ...
        if (fc_code) {
          ...
          console.log(fc_code);
          const data = await branch_employee_tax_by_status(fc_code.fc_code, selectedMonth, "Submitted");
  ...
  const handleSearch = async () => {
    try {
      ...
      setData(data);
      const submitedTaxData = await branch_employee_tax_by_status(fc_code.fc_code, selectedMonth, "Submitted");
      ...
      {branch_type === test_branch_type ? (
        <Box>
          ...
          <Box display="flex" justifyContent="flex-start" alignItems="center">
           ...
           <Box ml={15} display="flex" alignItems="right" gap={2}>
      <Button
